...
function updateReviewsIndicator(state) {
  ...
  segment.style.transform = `translate3d(${(logicalIndex / (total - 1)) * travel}%, 0, 0)`;
}
...
function bindReviewsSwipe(section) {
  ...
  const isMobileViewport = () => state.reviewVisibleCount === 1;
  ...
  state.viewport.addEventListener('pointercancel', (event) => {
...
function initCaseShowcaseSections() {
  ...
  roots.forEach((root) => {
    ...
    hint.textContent = 'Свайпните, чтобы посмотреть следующий кейс →';
...
function initHeader() {
  ...
  const openKgMm = () => {
    ...
    scheduleHeaderMeasure();
    const closeBtn = kgMm.querySelector('.kg-mm__close');
    const isCloseVisible = closeBtn && closeBtn.offsetParent !== null;
    if (isCloseVisible) {
      safeFocus(burger);
      return;
    }
...
function initKgxStoriesSlider() {
  ...
  if (!root) {
    if (isDev()) console.warn('[initKgxStoriesSlider] #kgx-stories-carousel section not found on this page');
    return;
  }
...
function initTelegramLeads() {
  ...
  forms.forEach((form) => {
    ...
    const sendLeadInBackground = (endpoint, fd) => {
      try {
        if (navigator.sendBeacon && navigator.sendBeacon(endpoint, fd)) return;
      } catch (e) {
      ...
      try {
        ...
        // Keep the thank-you redirect independent from transport errors.
      }
    };

    const sendLeadOnPageHide = (endpoint, sourceForm) => {
      let didSend = false;
      window.addEventListener('pagehide', () => {
        if (didSend) return;
        didSend = true;
        const fd = toFormData(sourceForm);
        sendLeadInBackground(endpoint, fd);
      }, { once: true });
    };
    ...
    const onSubmit = (e) => {
      ...
      sendLeadOnPageHide(endpoint, form);
      redirectToThanks();
    };
...
function initCounterAnimation() {
  ...
  if (!counters.length) {
    if (isDev()) console.warn('[initCounterAnimation] .kgx-stories__trust-bridge-counter not found');
    return;
  }
